...
class Quality(Simulation):
    ...
    def step(self, i: int, data: dict) -> None:
        men   = [100 * (float(val) / data['pool_size']) for val in range(data['pool_size'])]
        women = [100 * (float(val) / data['pool_size']) for val in range(data['pool_size'])]
...
def main(argv):
    ...
    plot_results(data['results'], pool_size, 'even', type = 'quality', xlabel = 'Quality')
